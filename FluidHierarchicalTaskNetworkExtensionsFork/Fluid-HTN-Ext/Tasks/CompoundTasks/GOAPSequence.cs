    ...
    public class GOAPSequence : Sequence
    {
        ...
        )
        {
            ...
            var start = ctx.Factory.Create<GOAPNode>();
            {
                ...
                start.Task = EmptyTask.Instance;
            }

            if (TryBuildGraph(ctx, start, leaves, Subtasks))
            {
                // GetCheapestLeaf can now return null if no path is found.
                GOAPNode? cheapestLeaf = GetCheapestLeaf(leaves);
                GeneratePlan(ctx, cheapestLeaf); // GeneratePlan now handles nullable GOAPNode
            }
        ...
        private static GOAPNode? GetCheapestLeaf(List<GOAPNode> leaves)
        {
        ...
        private static void FreeNode(IContext ctx, GOAPNode node)
        {
        ...
        private static List<ITask> GetSubset(IContext ctx, ITask currentTask, List<ITask> tasks)
        {
