...
namespace FluidHTN
{
    public static class DomainBuilderExt
    {
            ...
            where T : IContext
        {
            ...
            {
                throw new Exception(
                    "Pointer is not a Sequence, which is required for adding Always Succeed selectors!"
                );
            }
        }
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public static DB GOAPSequence<DB, T>(
            this DB builder,
            string name,
            params KeyValuePair<byte, byte>[] goal
        )
            where DB : BaseDomainBuilder<DB, T>
            where T : IContext
        {
        ...
        /// </summary>
        public static DB Repeat<DB, T>(
            this DB builder,
            string name,
            uint worldStateIndex,
            RepeatSequence.RepetitionType type = RepeatSequence.RepetitionType.Interleaved
        )
            where DB : BaseDomainBuilder<DB, T>
            where T : IContext
        {
            ...
            where P : IUtilityTask, IPrimitiveTask, new()
        {
            ...
            {
                throw new Exception(
                    "Pointer is not a Utility Selector, which is required for adding Utility Actions!"
                );
            }
        }
            ...
            where P : IGOAPTask, new()
        {
            ...
            {
                throw new Exception(
                    "Pointer is not a GOAP Sequence, which is required for adding GOAP Actions!"
                );
            }
        }
