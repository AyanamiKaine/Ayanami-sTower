...
/// </summary>
public class AddImageCloze : IUIComponent, IDisposable
{
    private class ClozeArea
    {
        ...
        public IBrush FillColor { get; set; } =
            new SolidColorBrush(Color.FromArgb(a: 180, r: 221, g: 176, b: 55));
    }
    ...
    /// <returns></returns>
    public AddImageCloze(World world)
    {
        _disposables.Add(
            _imagePathSubject
                ...
                (stackPanel) =>
                {
                                                ...
                                                (image) =>
                                                {
                                                    ...
                                                    _disposables.Add(
                                                        Disposable.Create(() =>
                                                        {
                                                            ...
                                                            {
                                                                image
                                                                    .RemoveOnPointerMoved(
                                                                        Image_PointerMoved
                                                                    )
                                                                    .RemoveOnPointerPressed(
                                                                        Image_PointerPressed
                                                                    )
                                                                    .RemoveOnPointerReleased(
                                                                        Image_PointerReleased
                                                                    );
                                                            }
                                                        })
                                                    );
                    ...
                    stackPanel.Child<TextBlock>(
                        (textBlock) =>
                        {
                            ...
                            textBlock.SetText(
                                "Mark a area in the image to create a cloze for it. To delete it again simply right click and select remove"
                            );
                    ...
                    stackPanel.Child<Button>(
                        (button) =>
                        {
                            ...
                            createButtonClickedHandler = (sender, args) =>
                            {
                                ...
                                if (
                                    string.IsNullOrEmpty(nameTextBox.GetText())
                                    || string.IsNullOrEmpty(ImagePath)
                                )
                                {
                                    nameTextBox!.SetWatermark(
                                        "Name and image with cloze areas are required"
                                    );
                                    return;
                                }
                                ...
                                if (clozeAreas.Count == 0)
                                {
                                    var cd = new ContentDialog()
                                    {
                                        Title = "Missing Areas",
                                        Content =
                                            "Your image does not currently have hidding any areas defined, they are required",
                                ...
                                if (_root.IsValid())
                                {
                                    ...
                                    var imageClozeAreas = clozeAreas
                                        .Select(area => new ImageClozeArea
                                        {
                                            X = area.X,
                                            Y = area.Y,
                                            Width = area.Width,
                                    ...
                                    // Create the SpacedRepetitionImageCloze
                                    world
                                        .Get<ObservableCollection<SpacedRepetitionItem>>()
                                        .Add(
                                            new SpacedRepetitionImageCloze
                                            {
                                                ...
                                                SpacedRepetitionItemType =
                                                    SpacedRepetitionItemType.ImageCloze,
                                            }
                                        );

                                    // Clear rectangles from canvas
                                    if (canvasEntity.IsValid())
                                    {
                                        var canvas = canvasEntity.Get<Canvas>();

                                        // Remove all rectangle elements from the canvas
                                        // We need to create a copy of the children to avoid collection modification issues
                                        var rectanglesToRemove = canvas
                                            .Children.OfType<Rectangle>()
                                            .ToList();
    ...
    private void Image_PointerPressed(object? sender, Avalonia.Input.PointerPressedEventArgs e)
    {
        ...
        if (e.GetCurrentPoint(sender as Control).Properties.IsLeftButtonPressed)
        {
            ...
            selectionRectangle = canvasEntity
                .CsWorld()
                .UI<Rectangle>(
                    (rect) =>
                    {
                        rect.SetStroke(new SolidColorBrush(Colors.Red))
    ...
    private void Image_PointerReleased(object? sender, Avalonia.Input.PointerReleasedEventArgs e)
    {
        ...
        var rectColor = new SolidColorBrush(Color.FromArgb(a: 180, r: 221, g: 176, b: 55));
        ...
        if (width > 10 && height > 10)
        {
            ...
            var rectangleEntity = world.UI<Rectangle>(
                (rect) =>
                {
                    rect.SetWidth(width).SetHeight(height).SetFill(rectColor);
                                    ...
                                    menuItem
                                        ...
                                        .OnClick(
                                            (_, _) =>
                                            {
                                                canvasEntity
                                                    .Get<Canvas>()
                                                    .Children.Remove(rect.Get<Rectangle>());

                                                // Remove the corresponding cloze area
                                                var clozeToRemove = clozeAreas.FirstOrDefault(ca =>
                                                    Math.Abs(
                                                        ca.X - Canvas.GetLeft(rect.Get<Rectangle>())
                                                    ) < 0.1
                                                    && Math.Abs(
                                                        ca.Y - Canvas.GetTop(rect.Get<Rectangle>())
                                                    ) < 0.1
                                                    && Math.Abs(
                                                        ca.Width - rect.Get<Rectangle>().Width
                                                    ) < 0.1
                                                    && Math.Abs(
                                                        ca.Height - rect.Get<Rectangle>().Height
                                                    ) < 0.1
                                                );
