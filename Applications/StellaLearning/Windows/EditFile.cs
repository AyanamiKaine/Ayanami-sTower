    ...
    private Entity DefineWindowContents(World world)
    {
        ...
        return world
            .UI<StackPanel>(
                (stackPanel) =>
                {
                    UIBuilder<TextBlock> validationTextBlock = world.UI<TextBlock>(
                        (textBlock) =>
                        {
                            textBlock.SetText("");
                    ...
                    void SaveData()
                    {
                        if (nameTextBox is null || questionTextBox is null || filePath is null)
                        {
                            return;
                        }
                        ...
                        Dispatcher.UIThread.InvokeAsync(async () =>
                        {
                            await StatsTracker.Instance.UpdateTagsForItemAsync(
                                spacedRepetitionFile.Uid,
                                spacedRepetitionFile.Tags
                            );
                        });
