...
public class KnowledgeVaultPage : IUIComponent
{
    ...
    public KnowledgeVaultPage(World world)
    {
        ...
        */
        ...
        _root.AddDefaultStyling(
            (vaultPage) =>
            {
                // Here we are adding default styling when the page is a child of a NavigationView
                if (vaultPage.Parent() != 0 && vaultPage.Parent().Has<NavigationView>())
                {
        ...
        var vaultContent = _root.UI<TextBlock>(
            (textBlock) =>
            {
                textBlock.SetText("VaultContent").SetRow(0).SetColumn(0);
            }
        );
        ...
        [
            ...
            new(
                ...
                "The ANSI standard goes in detail over the refrence implementation of common lisp",
        ...
        var contentTemplate = new FuncDataTemplate<Content>(
            (item, nameScope) =>
            {
                var grid = new Grid
                {
                ...
                */
                ...
                priorityTextBlock.Bind(
                    TextBlock.TextProperty,
                    new Binding("Priority") { StringFormat = "Priority: {0}" }
                );
                ...
                var priorityTooltipTextBlock = new TextBlock
                {
                    ...
                    MaxWidth = 200, // Set a maximum width for wrapping
                    Text =
                        "Priority shows the importance, it determines in which order content will be consumed.",
                };
        ...
        var scrollViewer = _root.UI<ScrollViewer>(
            (scrollViewer) =>
            {
                scrollViewer.SetRow(1).SetColumnSpan(3);

                var vaultItems = scrollViewer.Child<ListBox>(
                    (listBox) =>
                    {
                        listBox
