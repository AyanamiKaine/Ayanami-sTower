...
public class Engine
{
    ...
    private void InitDefaultSystems()
    {
        ...
        World.System<Orientation, RotationSpeed2D, DeltaTime>(name: "RotateSystem")
                    .Kind(phase: Ecs.OnUpdate) // Use Ecs.OnUpdate directly if Phases["OnUpdate"] is not defined or needed
                    .TermAt(termIndex: 2).Singleton()
                    .Each(callback: (ref Orientation orientation, ref RotationSpeed2D speed, ref DeltaTime dt) => // Use Field<T> for component access
                    {
                        float deltaAngleRadians = speed.Speed * dt.DeltaSeconds;
                        Quaternion deltaRotation = Quaternion.CreateFromAxisAngle(axis: Vector3.UnitZ, angle: deltaAngleRadians);
                        ...
                        orientation.Value = Quaternion.Normalize(value: orientation.Value * deltaRotation); // Normalize to prevent drift
                    });

        World.System<Position2D, Size2D, Orientation, RgbaColor, Renderer>(name: "Render2DBoxSystem") // Query Orientation instead of Angle
            .Kind(phase: Phases["OnRender"])
            .TermAt(termIndex: 4).Singleton()
            .Each(callback: (ref Position2D pos, ref Size2D size, ref Orientation orientation, ref RgbaColor color, ref Renderer renderer) => // Use Orientation
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    transformedCorners[j] = Vector2.Transform(value: corners[j], rotation: currentOrientation) + center;
                }


                // Draw lines (same as before)
                renderer.DrawLine(x1: transformedCorners[0].X, y1: transformedCorners[0].Y, x2: transformedCorners[1].X, y2: transformedCorners[1].Y);
                renderer.DrawLine(x1: transformedCorners[1].X, y1: transformedCorners[1].Y, x2: transformedCorners[2].X, y2: transformedCorners[2].Y);
                renderer.DrawLine(x1: transformedCorners[2].X, y1: transformedCorners[2].Y, x2: transformedCorners[3].X, y2: transformedCorners[3].Y);
                renderer.DrawLine(x1: transformedCorners[3].X, y1: transformedCorners[3].Y, x2: transformedCorners[0].X, y2: transformedCorners[0].Y);

            });

        World.System<Orientation, RotationSpeed3D, DeltaTime>(name: "RotateCubeSystem")
            .Kind(phase: Ecs.OnUpdate)
            .TermAt(termIndex: 2).Singleton() // DeltaTime is a singleton
            .Each(callback: (ref Orientation orientation, ref RotationSpeed3D speed, ref DeltaTime dt) =>
            {
                // Create rotations for each axis based on speed and delta time
                Quaternion deltaX = Quaternion.CreateFromAxisAngle(axis: Vector3.UnitX, angle: speed.SpeedRadPerSec.X * dt.DeltaSeconds);
                Quaternion deltaY = Quaternion.CreateFromAxisAngle(axis: Vector3.UnitY, angle: speed.SpeedRadPerSec.Y * dt.DeltaSeconds);
                Quaternion deltaZ = Quaternion.CreateFromAxisAngle(axis: Vector3.UnitZ, angle: speed.SpeedRadPerSec.Z * dt.DeltaSeconds);

                // Combine the deltas and apply to the current orientation
                // Order matters: ZYX is a common convention
                orientation.Value = Quaternion.Normalize(value: orientation.Value * deltaZ * deltaY * deltaX);
            });

        World.System<ProjectedMesh, MeshGeometry, Orientation, Position3D, Camera, Window>(name: "ProjectCustomMeshSystem")
            .Kind(phase: Phases["PreRender"])
            .TermAt(termIndex: 4).Singleton() // Camera is singleton
            .TermAt(termIndex: 5).Singleton() // Window is singleton
            .Each(callback: (ref ProjectedMesh projected, ref MeshGeometry geometry, ref Orientation orientation, ref Position3D position, ref Camera camera, ref Window windowSize) =>
            {
                ...
                Matrix4x4 rotationMatrix = Matrix4x4.CreateFromQuaternion(quaternion: orientation.Value);
                Matrix4x4 translationMatrix = Matrix4x4.CreateTranslation(position: position.Value);
                ...
                Matrix4x4 modelViewMatrix = Matrix4x4.Multiply(value1: modelMatrix, value2: viewMatrix);
                Matrix4x4 mvpMatrix = Matrix4x4.Multiply(value1: modelViewMatrix, value2: projectionMatrix); // MVP = (Model * View) * Projection
                ...
                for (int i = 0; i < geometry.BaseVertices.Length; i++)
                {
                    ...
                    Vector4 clipSpaceVertex = Vector4.Transform(position: geometry.BaseVertices[i], matrix: mvpMatrix);
        ...
        World.System<Position3D, Size3D, Orientation, RgbaColor, Camera, Window, Renderer>(name: "RenderBoxWireframeSystem")
            .With<Box>() // <--- Query for entities with the Box tag!
            .Kind(phase: Phases["OnRender"])
            .TermAt(termIndex: 4).Singleton() // Camera
            .TermAt(termIndex: 5).Singleton() // WindowSize
            .TermAt(termIndex: 6).Singleton() // Renderer
            .Each(callback: (ref Position3D pos, ref Size3D size, ref Orientation orientation, ref RgbaColor color, ref Camera camera, ref Window window, ref Renderer renderer) =>
            {
                if (renderer == null) return;
            ...
            new(-hw, -hh,  hd), new( hw, -hh,  hd), new( hw,  hh,  hd), new(-hw,  hh,  hd)  // Front face indices 4-7
                ...
                Matrix4x4 modelMatrix = Matrix4x4.CreateFromQuaternion(quaternion: orientation.Value) * Matrix4x4.CreateTranslation(position: pos.Value);
                ...
                for (int i = 0; i < 8; i++)
                {
                    Vector4 clipSpaceVertex = Vector4.Transform(position: localVertices[i], matrix: mvpMatrix);
                ...
                for (int i = 0; i < 12; i++)
                {
                    ...
                    {
                        renderer.DrawLine(x1: screenVertices[i1].X, y1: screenVertices[i1].Y, x2: screenVertices[i2].X, y2: screenVertices[i2].Y);
                    }
                }
            });

        World.System<ProjectedMesh, MeshGeometry, RgbaColor, Renderer>(name: "RenderCustomMeshWireframeSystem")
            .Kind(phase: Phases["OnRender"])
            .TermAt(termIndex: 3).Singleton() // Renderer is singleton
            .Each(callback: (ref ProjectedMesh projected, ref MeshGeometry geometry, ref RgbaColor color, ref Renderer renderer) =>
            {
    ...
    private void InitComponents()
    {
        ...
        World.Component<Vector2>(name: "Vector2")
            .Member<float>(name: "X")
            .Member<float>(name: "Y");

        World.Component<Vector3>(name: "Vector3")
            .Member<float>(name: "X")
            .Member<float>(name: "Y")
            .Member<float>(name: "Z");


        World.Component<Vector4>(name: "Vector4")
            .Member<float>(name: "X")
            .Member<float>(name: "Y")
            .Member<float>(name: "Z")
            .Member<float>(name: "W");

        World.Component<Plane>(name: "Plane")
            .Member<Vector3>(name: "Normal")
            .Member<float>(name: "D");

        World.Component<Matrix3x2>(name: "Matrix3x2")
        ...
        World.Component<Matrix4x4>(name: "Matrix4x4")
            .Member<float>(name: "M11")
            .Member<float>(name: "M12")
        ...
        World.Component<Quaternion>(name: "Quaternion")
            .Member<float>(name: "X")
            .Member<float>(name: "Y")
            .Member<float>(name: "Z")
            .Member<float>(name: "W");

        World.Component<Size2D>(name: "Size2D")
            .Member<Vector2>(name: "Value");

        World.Component<Size3D>(name: "Size3D")
            .Member<Vector3>(name: "Value");

        World.Component<Orientation>(name: "Orientation")
            .Member<Quaternion>(name: "Value");

        World.Component<Rotation3D>(name: "Rotation3D")
            .Member<Vector3>(name: "Value");

        World.Component<Position2D>(name: "Position2D")
            .Member<Vector2>(name: "Value");

        World.Component<Position3D>(name: "Position3D")
            .Member<Vector3>(name: "Value");

        World.Component<RotationSpeed2D>(name: "RotationSpeed2D")
            .Member<float>(name: "Speed");

        World.Component<RotationSpeed3D>(name: "RotationSpeed3D")
            .Member<Vector3>(name: "SpeedRadPerSec");

        World.Component<ECS.Point>(name: "Point")
            .Member<Vector2>(name: "Value");

        World.Component<Line>(name: "Line")
            .Member<ECS.Point>(name: "Start")
            .Member<ECS.Point>(name: "End");

        World.Component<RgbaColor>(name: "Color")
            .Member<byte>(name: "R")
            .Member<byte>(name: "G")
            .Member<byte>(name: "B")
            .Member<byte>(name: "A");

        World.Component<ProjectionType>(name: "ProjectionType")
            .Constant(name: "Perspective", value: ProjectionType.Perspective)
            .Constant(name: "Orthographic", value: ProjectionType.Orthographic);

        World.Component<Camera>(name: "Camera")
    ...
    private void InitRenderSystems()
    {
        World.System<Renderer>()
            .Kind(phase: Phases["PreRender"])
            .Each(callback: (Entity _, ref Renderer renderer) =>
            {
                renderer.Clear();
            });
