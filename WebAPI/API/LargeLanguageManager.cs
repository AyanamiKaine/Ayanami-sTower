...
public partial class LargeLanguageManager // Removed 'sealed'
{
    ...
    )
    {
        ...
        var apiKey =
            Environment.GetEnvironmentVariable("GEMINI_API") ?? _configuration["Gemini:ApiKey"];

        if (string.IsNullOrEmpty(apiKey))
        {
            _logger.LogCritical(
                "Gemini API Key not found. Set GEMINI_API environment variable or Gemini:ApiKey in configuration."
            );
            throw new InvalidOperationException("Gemini API Key not configured.");
        }

        // Read feature flag from Configuration instead of World/Settings
        _enableLargeLanguageFeatures = _configuration.GetValue(
            "Features:EnableLargeLanguage",
            true
        ...
        try
        {
            ...
            var textModelName =
                _configuration["Gemini:TextModel"] ?? GoogleAIModels.Gemini2FlashExp;
            ...
            _logger.LogInformation(
                "LargeLanguageManager initialized with TextModel: {TextModel}, ImageModel: {ImageModel}",
                textModelName,
                imageModelName
            );
        }
    ...
    public async Task<string?> GetTextResponseAsync(string prompt)
    {
        ...
        {
            _logger.LogWarning(
                "Large language features are disabled. Skipping GetTextResponseAsync."
            );
            return null;
        }
    ...
    public async Task<ContentMetadata?> GenerateMetaDataBasedOnFileAsync(
    ...
    )
    {
        ...
        {
            _logger.LogWarning(
                "Large language features are disabled. Skipping GenerateMetaDataBasedOnFile."
            );
            return null;
        }
        ...
        {
            _logger.LogError(
                "StoredPath is missing in FileMetadata for ID {FileId}",
                fileMetadata.Id
            );
        ...
        {
            _logger.LogError(
                "Secure file path does not exist: {SecureFilePath} (File ID: {FileId})",
                secureFilePath,
                fileMetadata.Id
            );
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Decrypting per-file key using master key for File ID {FileId}",
                fileMetadata.Id
            );
            ...
            _logger.LogInformation(
                "Successfully decrypted file key for File ID {FileId}",
                fileMetadata.Id
            );
        }
        catch (FormatException ex)
        {
            _logger.LogError(
                ex,
                "Failed to decode Base64 encryption key/IV for File ID {FileId}",
                fileMetadata.Id
            );
            return null;
        }
        catch (CryptographicException cryptoEx)
        {
            _logger.LogError(
                cryptoEx,
                "Failed to decrypt file key using master key for File ID {FileId}. Master key might be incorrect or data corrupted.",
                fileMetadata.Id
            );
            return null;
        }
        catch (Exception ex) // Catch broader exceptions during setup
        {
            _logger.LogError(
                ex,
                "Unexpected error during decryption setup for File ID {FileId}",
                fileMetadata.Id
            );
            return null;
        }
        ...
        bool canExtractText =
            _convertibleToTextExtensions.Contains(extension ?? "") || (extension == ".txt");
        ...
        try
        {
            ...
            if (useFileApi) // File API preferred for its native support
            {
                ...
                tempDecryptedFilePath = Path.Combine(
                    Path.GetTempPath(),
                    $"{Guid.NewGuid()}{Path.GetExtension(fileMetadata.OriginalFileName)}"
                ); // Use original extension
                _logger.LogInformation(
                    "Decrypting file {FileId} to temporary path {TempPath} for File API upload.",
                    fileMetadata.Id,
                ...
                _logger.LogInformation(
                    "Uploading temporary decrypted file '{FileName}' using File API (File ID: {FileId})",
                    Path.GetFileName(tempDecryptedFilePath),
                    fileMetadata.Id
                );
                remoteFile = await _textModel.Files.UploadFileAsync(tempDecryptedFilePath);
                _logger.LogInformation(
                    "Temporary decrypted file uploaded successfully. Remote File Name: {RemoteFileName}",
                    remoteFile.Name
                );
                ...
                _logger.LogInformation(
                    "Sending request to AI model using File API (File ID: {FileId})",
                    fileMetadata.Id
                );
            ...
            {
                _logger.LogInformation(
                    "Attempting direct text extraction from decrypted stream for File ID {FileId}",
                    fileMetadata.Id
                );
                // For plain text:
                if (extension == ".txt" || _convertibleToTextExtensions.Contains(extension ?? ""))
                {
                    ...
                    _logger.LogInformation(
                        "Extracted {Length} characters from decrypted text stream for File ID {FileId}",
                    ...
                    {
                        _logger.LogWarning(
                            "Text extraction yielded empty content for File ID {FileId}",
                            fileMetadata.Id
                        );
                        return null;
                    }
                    ...
                    _logger.LogInformation(
                        "Sending request to AI model using extracted text (File ID: {FileId})",
                        fileMetadata.Id
                    );
                ...
                {
                    _logger.LogWarning(
                        "Text extraction not implemented for file type '{Extension}' for File ID {FileId}",
                        extension,
                        fileMetadata.Id
                    );
            ...
            {
                _logger.LogWarning(
                    "File type '{Extension}' for File ID {FileId} is not supported by File API and not configured for text extraction.",
                    extension,
                    fileMetadata.Id
                );
                return null;
        ...
        catch (CryptographicException cryptoEx)
        {
            _logger.LogError(
                cryptoEx,
                "Cryptography error during file decryption for File ID {FileId}",
                fileMetadata.Id
            );
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error during AI processing or decryption for File ID '{FileId}'",
                fileMetadata.Id
            );
            return null;
        }
        finally
        {
            ...
            if (decryptedFileKey != null)
                Array.Clear(decryptedFileKey, 0, decryptedFileKey.Length);
            ...
            if (tempDecryptedFilePath != null && File.Exists(tempDecryptedFilePath))
            {
                try
                {
                    ...
                    _logger.LogInformation(
                        "Deleted temporary decrypted file: {TempPath}",
                        tempDecryptedFilePath
                    );
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(
                        ex,
                        "Failed to delete temporary decrypted file: {TempPath}",
                        tempDecryptedFilePath
                    );
                }
            }
            if (remoteFile != null)
            {
                try
                {
                    ...
                    _logger.LogInformation(
                        "Deleted remote file: {RemoteFileName}",
                        remoteFile.Name
                    );
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(
                        ex,
                        "Failed to delete remote file: {RemoteFileName}",
                        remoteFile.Name
                    );
                }
            }
    ...
    public async Task<List<string>> GetImageTagsAsync(FileMetadata fileMetadata, int maxTags = 4) // Changed signature
    {
        ...
        if (string.IsNullOrEmpty(fileMetadata.StoredPath))
        { /* ... */
            return [];
        }
        if (!File.Exists(fileMetadata.StoredPath))
        { /* ... */
            return [];
        }
        ...
        {
            _logger.LogDebug(
                "Decoding/Decrypting keys for image tag generation (File ID {FileId})",
                fileMetadata.Id
            );
            byte[] encryptedKeyBytes = Convert.FromBase64String(fileMetadata.EncryptedFileKey);
            fileContentIv = Convert.FromBase64String(fileMetadata.EncryptionIV);
            decryptedFileKey = EncryptionHelper.DecryptData(_masterKey, encryptedKeyBytes);
            _logger.LogInformation(
                "Successfully decrypted file key for image (File ID {FileId})",
                fileMetadata.Id
            );
        }
        catch (Exception ex) // Catch format, crypto, etc.
        {
            _logger.LogError(
                ex,
                "Failed decryption setup for image tag generation (File ID {FileId})",
                fileMetadata.Id
            );
            return [];
        }
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Reading decrypted image data into memory for {FileId}",
                fileMetadata.Id
            );
            await cryptoStream.CopyToAsync(memoryStream);
            decryptedImageBytes = memoryStream.ToArray();
            _logger.LogInformation(
                "Successfully decrypted image data ({Length} bytes) for File ID {FileId}",
                decryptedImageBytes.Length,
                fileMetadata.Id
            );
        }
        catch (CryptographicException cryptoEx)
        {
            _logger.LogError(
                cryptoEx,
                "Cryptography error during image file decryption for File ID {FileId}",
                fileMetadata.Id
            );
            return [];
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error reading or decrypting image file for File ID {FileId}",
                fileMetadata.Id
            );
            return [];
        }
        ...
        try
        {
            ...
            request.AddInlineData(
                imageDataBase64,
                fileMetadata.ContentType ?? "image/jpeg",
                true,
                "user"
            ); // Use stored content type, fallback if null

            _logger.LogInformation(
                "Sending request for image tags using inline decrypted data (File ID: {FileId})",
                fileMetadata.Id
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error sending decrypted image data or generating tags for File ID '{FileId}'",
                fileMetadata.Id
            );
            return [];
        }
    ...
    )
    {
        ...
        _logger.LogInformation(
            "Sending combined prompt (Extracted Text + Question) to AI model..."
        );
        try
        {
    ...
    public async Task<ContentMetadata?> GenerateMetaDataFromUrlAsync(string url, int maxTags = 5)
    {
        ...
        {
            _logger.LogWarning(
                "Large language features are disabled. Skipping GenerateMetaDataFromUrlAsync."
            );
            return null;
        }
    ...
    private async Task<string?> FetchAndCleanHtmlAsync(string url)
    {
        ...
        {
            _logger.LogError($"HTTP error fetching URL '{url}': {ex.Message}");
            return null;
        }
        catch (TaskCanceledException ex)
        {
