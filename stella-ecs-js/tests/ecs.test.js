...
class Galaxy { }
class StarSystem { }
class Name { constructor(value) { this.value = value; } }
...
describe("Galaxy Map Simulation", () => {
    function setupGalaxy() {
        ...
        // Create Galaxies
        const milkyWay = world.createEntity().set(new Galaxy()).set(new Name("Milky Way"));
        const andromeda = world.createEntity().set(new Galaxy()).set(new Name("Andromeda"));

        // Create Star Systems
        const sol = world.createEntity().set(new StarSystem()).set(new Name("Sol")).set(new Position(1, 1));
        const alphaCentauri = world.createEntity().set(new StarSystem()).set(new Name("Alpha Centauri")).set(new Position(5, 8));
        const sirius = world.createEntity().set(new StarSystem()).set(new Name("Sirius")).set(new Position(10, -4));
        const triangulum = world.createEntity().set(new StarSystem()).set(new Name("Triangulum")).set(new Position(100, 100));
        ...
        alphaCentauri.connectTo(sirius, { type: 'hyperspaceLane', distance: 8.6 });

        return { world, staticComponents, milkyWay, andromeda, sol, alphaCentauri, sirius, triangulum };
    }

    test("should correctly model and query galaxy and star system relationships", () => {
        const { world, milkyWay, andromeda, sol, alphaCentauri, sirius } = setupGalaxy();
        ...
        const solToAlphaCentauri = solConnections.find(c => c.neighbor == alphaCentauri.id);
    ...
    test("should correctly serialize and deserialize a galaxy map", () => {
        const { world, staticComponents, milkyWay, andromeda, sol, alphaCentauri } = setupGalaxy();
        ...
        expect(newWorld.getComponent(milkyWay.id, Name).value).toBe("Milky Way");
