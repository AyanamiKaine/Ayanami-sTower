...
namespace Avalonia.Flecs.Controls.ECS
{
    ...
    public class ECSDockPanel : IFlecsModule
    {
        ...
        public void InitModule(World world)
        {
            ...
            world
                .Component<DockPanel>("DockPanel")
                .OnSet(
                    (Entity e, ref DockPanel dockPanel) =>
                    {
                        if (!e.Has<object>())
                        {
                            e.Set<object>(dockPanel);
                        }
                        else if (e.Get<object>().GetType() == typeof(DockPanel))
                        {
                            e.Set<object>(dockPanel);
                        }
                ...
                .OnRemove((Entity e, ref DockPanel _) => e.Remove<Panel>());
        }
    }
}
