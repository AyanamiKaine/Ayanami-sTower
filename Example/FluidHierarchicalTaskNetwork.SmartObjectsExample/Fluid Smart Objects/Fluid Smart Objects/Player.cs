    ...
    public class Player
    {
        ...
        private Domain<AIContext> BuildDomain()
        {
            return new DomainBuilder<AIContext>("Player")
                ...
                .Select("Walk about")
                    .Action("Walk about action")
                        .Condition("At location 2", context => context.GetState(AIWorldState.Location) == 2)
                        .Do(static _ =>
                        {
                            Console.WriteLine("Ariving at location 3");
                            return TaskStatus.Success;
                        })
                        .Effect("At location 3", EffectType.PlanAndExecute, (context, type) => context.SetState(AIWorldState.Location, 3, type))
                    .End()
                .End()
                .Select("Idle")
                    .Action("Idle action")
