...
public class MinimalApp : App // Inherit from App
{
    ...
    protected override bool OnInit(string[] args)
    {
        ...
        */
        ...
        AppEntity.Observe(
            (ref WindowResize windowResize) =>
            {
                Console.WriteLine(
                    $"Window was resized. New Height: {windowResize.Height} New Width: {windowResize.Width}"
                );
            }
        );

        AppEntity.Observe(
            (ref KeyDownEvent keyDownEvent) =>
            {
        ...
        AppEntity.Observe(
            (ref MouseMotionEvent mouseMotionEvent) =>
            {
                Console.WriteLine(
                    $"Mouse is moving: X: {mouseMotionEvent.X} Y:{mouseMotionEvent.Y} YRel: {mouseMotionEvent.YRel} XRel: {mouseMotionEvent.XRel}"
                );
            }
        );

        AppEntity.Observe(
            (ref MouseButtonDownEvent mouseButtonDownEvent) =>
            {
                Console.WriteLine(
                    $"Mouse button was pressed down: {mouseButtonDownEvent.MouseButton}"
                );
            }
        );

        AppEntity.Observe(
            (ref MouseWheelEvent mouseWheelEvent) =>
            {
                Console.WriteLine(
                    $"Mouse wheel is moving:  DirectionType: {mouseWheelEvent.Direction} ScrollX: {mouseWheelEvent.ScrollX} ScrollY: {mouseWheelEvent.ScrollY}"
                );
            }
        );

        AppEntity.Observe(
            (ref WindowMovedEvent windowMovedEvent) =>
            {
                Console.WriteLine(
                    $"Window was moved:  NewY: {windowMovedEvent.Y} NewX: {windowMovedEvent.X}"
                );
            }
        );
