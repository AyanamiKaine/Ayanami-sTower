...
namespace FluidHTN
{
        ...
        where T : IContext
    {
        ...
        /// <returns></returns>
        public DB CompoundTask<P>(string name)
            where P : ICompoundTask, new()
        {
            var parent = new P();
        ...
        /// <returns></returns>
        public DB CompoundTask<P>(string name, P task)
            where P : ICompoundTask
        {
            if (task != null)
            {
        ...
        /// <param name="name">The name given to the task, mainly for debug/display purposes</param>
        /// <returns></returns>
        public DB PrimitiveTask<P>(string name)
            where P : IPrimitiveTask, new()
        {
            if (Pointer is ICompoundTask compoundTask)
            {
        ...
        */
        ...
        public Domain<T> Build()
        {
            ...
            {
                throw new Exception(
                    $"The domain definition lacks one or more End() statements. Pointer is '{Pointer.Name}', but expected '{_domain.Root.Name}'."
                );
            }
