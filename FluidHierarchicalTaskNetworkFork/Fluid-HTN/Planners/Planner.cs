        ...
        where T : IContext
    {
        ...
        public void Tick(Domain<T> domain, T ctx, bool allowImmediateReplan = true)
        {
            ...
            if (CanSelectNextTaskInPlan(ctx))
            {
                ...
                if (!SelectNextTaskInPlan(domain, ctx))
                {
                    return;
                }
            }
            ...
            if (
                ctx.PlannerState.CurrentTask is IPrimitiveTask task
                && !TryTickPrimitiveTaskOperator(domain, ctx, task, allowImmediateReplan)
            )
            {
                return;
            }
        ...
        /// <param name="ctx"></param>
        /// <returns></returns>
        private static bool ShouldFindNewPlan(T ctx)
        {
            return ctx.IsDirty
                || (ctx.PlannerState.CurrentTask == null && ctx.PlannerState.Plan.Count == 0);
        }

        private bool TryFindNewPlan(
            Domain<T> domain,
            T ctx,
            out DecompositionStatus decompositionStatus
        )
        {
        ...
        /// </summary>
        private static Queue<PartialPlanEntry> PrepareDirtyWorldStateForReplan(T ctx)
        {
            if (!ctx.IsDirty)
            {
                return null;
        ...
        private static Queue<PartialPlanEntry> CacheLastPartialPlan(T ctx)
        {
        ...
        private static void RestoreLastPartialPlan(
            T ctx,
        ...
        private static void OnFoundNewPlan(T ctx, Queue<ITask> newPlan)
        {
            if (
                ctx.PlannerState.OnReplacePlan != null
                && (ctx.PlannerState.Plan.Count > 0 || ctx.PlannerState.CurrentTask != null)
            )
            {
                ctx.PlannerState.OnReplacePlan.Invoke(
                    ctx.PlannerState.Plan,
                    ctx.PlannerState.CurrentTask,
                    newPlan
                );
            }
            ...
            if (
                ...
                && ctx.PlannerState.CurrentTask is IPrimitiveTask t
            )
            {
        ...
        /// </summary>
        /// <param name="ctx"></param>
        private static void CopyMtrToLastMtr(T ctx)
        {
            if (ctx.MethodTraversalRecord != null)
            {
                ctx.LastMTR.Clear();
                ctx.LastMTR.AddRange(ctx.MethodTraversalRecord);

                if (ctx.DebugMTR)
                {
                    ctx.LastMTRDebug.Clear();
                    ctx.LastMTRDebug.AddRange(ctx.MTRDebug);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="ctx"></param>
        private static void RestoreLastMethodTraversalRecord(T ctx)
        {
            if (ctx.LastMTR.Count > 0)
            {
                ctx.MethodTraversalRecord.Clear();
                ctx.MethodTraversalRecord.AddRange(ctx.LastMTR);
                ctx.LastMTR.Clear();
                ...
                ctx.MTRDebug.Clear();
                ctx.MTRDebug.AddRange(ctx.LastMTRDebug);
                ctx.LastMTRDebug.Clear();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private static bool CanSelectNextTaskInPlan(T ctx)
        {
        ...
        /// <param name="allowImmediateReplan"></param>
        /// <returns></returns>
        private bool TryTickPrimitiveTaskOperator(
            Domain<T> domain,
            T ctx,
            IPrimitiveTask task,
            bool allowImmediateReplan
        )
        {
            if (task.Operator != null)
            {
                if (!IsExecutingConditionsValid(domain, ctx, task, allowImmediateReplan))
                {
                    return false;
                }
        ...
        private bool IsConditionsValid(T ctx)
        {
            foreach (var condition in ctx.PlannerState.CurrentTask.Conditions)
            {
                ...
                {
                    ctx.PlannerState.OnNewTaskConditionFailed?.Invoke(
                        ctx.PlannerState.CurrentTask,
                        condition
                    );
        ...
        /// <param name="allowImmediateReplan"></param>
        /// <returns></returns>
        private bool IsExecutingConditionsValid(
            Domain<T> domain,
            T ctx,
            IPrimitiveTask task,
            bool allowImmediateReplan
        ...
        /// </summary>
        /// <param name="ctx"></param>
        private static void AbortTask(T ctx, IPrimitiveTask task)
        {
            task?.Aborted(ctx);
        ...
        /// <param name="task"></param>
        /// <param name="allowImmediateReplan"></param>
        private void OnOperatorFinishedSuccessfully(
            Domain<T> domain,
            T ctx,
            IPrimitiveTask task,
            bool allowImmediateReplan
        ...
        /// </summary>
        /// <param name="ctx"></param>
        private static void ClearPlanForReplan(T ctx)
        {
            ctx.PlannerState.CurrentTask = null;
            ctx.PlannerState.Plan.Clear();
        ...
        /// <returns></returns>
        private static bool HasFailedToFindPlan(
            bool isTryingToReplacePlan,
            DecompositionStatus decompositionStatus,
            T ctx
        )
        {
            return ctx.PlannerState.CurrentTask == null
                && ctx.PlannerState.Plan.Count == 0
                && !isTryingToReplacePlan
                && (
