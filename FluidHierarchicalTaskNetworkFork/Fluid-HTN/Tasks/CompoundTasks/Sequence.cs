    ...
    public class Sequence : CompoundTask, IDecomposeAll
    {
        ...
        )
        {
            if (!task.IsValid(ctx))
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeTask:Failed:Task {task.Name}.IsValid returned false!",
                        ConsoleColor.Red
                    );
                }
            ...
            {
                return OnDecomposeCompoundTask(
                    ctx,
                    compoundTask,
                    taskIndex,
                    oldStackDepth,
            ...
            else if (task is PausePlanTask)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeTask:Return partial plan at index {taskIndex}!",
                        ConsoleColor.DarkBlue
                    );
                }

                ctx.HasPausedPartialPlan = true;
                ctx.PartialPlanQueue.Enqueue(
                    new PartialPlanEntry() { Task = this, TaskIndex = taskIndex + 1 }
                );
        ...
        )
        {
            ...
            if (ctx.LogDecomposition)
            {
                Log(
                    ctx,
                    $"Sequence.OnDecomposeTask:Pushed {task.Name} to plan!",
                    ConsoleColor.Blue
                );
            }
        ...
        )
        {
            ...
            if (status == DecompositionStatus.Rejected)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeCompoundTask:{status}: Decomposing {task.Name} was rejected.",
                        ConsoleColor.Red
                    );
                }
            ...
            if (status == DecompositionStatus.Failed)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeCompoundTask:{status}: Decomposing {task.Name} failed.",
                        ConsoleColor.Red
                    );
                }
            ...
            while (subPlan.Count > 0)
            {
                ...
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeCompoundTask:Decomposing {task.Name}:Pushed {p.Name} to plan!",
                        ConsoleColor.Blue
                    );
                }
            ...
            if (ctx.HasPausedPartialPlan)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeCompoundTask:Return partial plan at index {taskIndex}!",
                        ConsoleColor.DarkBlue
                    );
                }
        ...
        )
        {
            ...
            if (status == DecompositionStatus.Rejected)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeSlot:{status}: Decomposing {task.Name} was rejected.",
                        ConsoleColor.Red
                    );
                }
            ...
            if (status == DecompositionStatus.Failed)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeSlot:{status}: Decomposing {task.Name} failed.",
                        ConsoleColor.Red
                    );
                }
            ...
            while (subPlan.Count > 0)
            {
                ...
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeSlot:Decomposing {task.Name}:Pushed {p.Name} to plan!",
                        ConsoleColor.Blue
                    );
                }
            ...
            if (ctx.HasPausedPartialPlan)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Sequence.OnDecomposeSlot:Return partial plan at index {taskIndex}!",
                        ConsoleColor.DarkBlue
                    );
                }
