...
namespace FluidHTN.Compounds
{
    public class Selector : CompoundTask
    {
        ...
        private static bool BeatsLastMTR(IContext ctx, int taskIndex, int currentDecompositionIndex)
        {
        ...
        /// <returns></returns>
        protected override DecompositionStatus OnDecompose(
            IContext ctx,
            int startIndex,
        ...
        )
        {
            ...
            for (var taskIndex = startIndex; taskIndex < Subtasks.Count; taskIndex++)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecompose:Task index: {taskIndex}: {Subtasks[taskIndex]?.Name}"
                    );
                }
                ...
                if (ctx.LastMTR != null && ctx.LastMTR.Count > 0)
                {
                    if (ctx.MethodTraversalRecord.Count < ctx.LastMTR.Count)
                    {
                        ...
                        if (!BeatsLastMTR(ctx, taskIndex, currentDecompositionIndex))
                        {
        ...
        protected override DecompositionStatus OnDecomposeTask(
            IContext ctx,
            ITask task,
            int taskIndex,
            int[] oldStackDepth,
        ...
        )
        {
            if (!task.IsValid(ctx))
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeTask:Failed:Task {task.Name}.IsValid returned false!",
                        ConsoleColor.Red
                    );
                }
            ...
            result = Plan;
            var status =
                result.Count == 0 ? DecompositionStatus.Failed : DecompositionStatus.Succeeded;

            if (ctx.LogDecomposition)
            {
                Log(
                    ctx,
                    $"Selector.OnDecomposeTask:{status}!",
        ...
        protected override void OnDecomposePrimitiveTask(
            IContext ctx,
            IPrimitiveTask task,
            int taskIndex,
            int[] oldStackDepth,
        ...
        )
        {
            ...
            if (ctx.LogDecomposition)
            {
                Log(
                    ctx,
                    $"Selector.OnDecomposeTask:Pushed {task.Name} to plan!",
                    ConsoleColor.Blue
                );
            }
        ...
        protected override DecompositionStatus OnDecomposeCompoundTask(
            IContext ctx,
            ICompoundTask task,
            int taskIndex,
            int[] oldStackDepth,
        ...
        )
        {
            ...
            if (status == DecompositionStatus.Rejected)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeCompoundTask:{status}: Decomposing {task.Name} was rejected.",
                        ConsoleColor.Red
                    );
                }
            ...
            if (status == DecompositionStatus.Failed)
            {
                ...
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeCompoundTask:{status}: Decomposing {task.Name} failed.",
                        ConsoleColor.Red
                    );
                }
            ...
            while (subPlan.Count > 0)
            {
                ...
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeCompoundTask:Decomposing {task.Name}:Pushed {p.Name} to plan!",
                        ConsoleColor.Blue
                    );
                }
            ...
            if (ctx.HasPausedPartialPlan)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeCompoundTask:Return partial plan at index {taskIndex}!",
            ...
            if (ctx.LogDecomposition)
            {
                Log(
                    ctx,
                    $"Selector.OnDecomposeCompoundTask:{s}!",
        ...
        protected override DecompositionStatus OnDecomposeSlot(
            IContext ctx,
            Slot task,
            int taskIndex,
            int[] oldStackDepth,
        ...
        )
        {
            ...
            if (status == DecompositionStatus.Rejected)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeSlot:{status}: Decomposing {task.Name} was rejected.",
                        ConsoleColor.Red
                    );
                }
            ...
            if (status == DecompositionStatus.Failed)
            {
                ...
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeSlot:{status}: Decomposing {task.Name} failed.",
                        ConsoleColor.Red
                    );
                }
            ...
            while (subPlan.Count > 0)
            {
                ...
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        $"Selector.OnDecomposeSlot:Decomposing {task.Name}:Pushed {p.Name} to plan!",
                        ConsoleColor.Blue
                    );
                }
            ...
            if (ctx.HasPausedPartialPlan)
            {
                if (ctx.LogDecomposition)
                {
                    Log(
                        ctx,
                        "Selector.OnDecomposeSlot:Return partial plan!",
                        ConsoleColor.DarkBlue
                    );
                }
