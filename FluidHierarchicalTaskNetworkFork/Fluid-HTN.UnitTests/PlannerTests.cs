    ...
    public class PlannerTests
    {
        ...
        */
        ...
        public void OnReplacePlan_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnReplacePlan = (op, ct, p) =>
            {
                test = op.Count == 0 && ct != null && p.Count == 1;
            };
            var domain = new Domain<MyContext>("Test");
            var task1 = new Selector() { Name = "Test1" };
            var task2 = new Selector() { Name = "Test2" };
            var task3 = (IPrimitiveTask)
                new PrimitiveTask() { Name = "Sub-task1" }.AddCondition(
                    new FuncCondition<MyContext>("TestCondition", context => context.Done == false)
                );
        ...
        public void OnNewTaskConditionFailed_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnNewTaskConditionFailed = (t, c) =>
            {
            ...
            task3.AddEffect(
                new ActionEffect<MyContext>(
                    ...
                    (context, type) => context.Done = true
                )
            );
            task4.SetOperator(new FuncOperator<MyContext>((context) => TaskStatus.Continue));
            domain.Add(domain.Root, task1);
        ...
        public void OnStopCurrentTask_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnStopCurrentTask = (t) =>
            {
        ...
        public void OnCurrentTaskCompletedSuccessfully_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnCurrentTaskCompletedSuccessfully = (t) =>
            {
                test = t.Name == "Sub-task1";
            };
            var domain = new Domain<MyContext>("Test");
        ...
        public void OnApplyEffect_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnApplyEffect = (e) =>
            {
            ...
            var task3 = (IPrimitiveTask)
                new PrimitiveTask() { Name = "Sub-task1" }.AddCondition(
                    new FuncCondition<MyContext>(
                        "TestCondition",
                        context => !context.HasState(MyWorldState.HasA)
                    )
                );
            var task4 = new PrimitiveTask() { Name = "Sub-task2" };
            task3.SetOperator(new FuncOperator<MyContext>((context) => TaskStatus.Success));
        ...
        public void OnCurrentTaskFailed_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnCurrentTaskFailed = (t) =>
            {
                test = t.Name == "Sub-task";
            };
            var domain = new Domain<MyContext>("Test");
        ...
        public void OnCurrentTaskContinues_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnCurrentTaskContinues = (t) =>
            {
                test = t.Name == "Sub-task";
            };
            var domain = new Domain<MyContext>("Test");
        ...
        public void OnCurrentTaskExecutingConditionFailed_ExpectedBehavior()
        {
            ...
            ctx.Init();
            var planner = new Planner<MyContext>();
            ctx.PlannerState.OnCurrentTaskExecutingConditionFailed = (t, c) =>
            {
            ...
            task2.AddExecutingCondition(
                new FuncCondition<MyContext>("TestCondition", context => context.Done)
            );
            domain.Add(domain.Root, task1);
            domain.Add(task1, task2);
        ...
        public void FindPlanIfWorldStateChangeAndOperatorIsContinuous_ExpectedBehavior()
        {
            ...
            actionA.AddCondition(
                new FuncCondition<MyContext>(
                    "Can choose A",
                    context => context.GetState(MyWorldState.HasA) == 1
                )
            );
            actionA.SetOperator(new MyOperator());
            var actionB = new PrimitiveTask() { Name = "Test Action B" };
            actionB.AddCondition(
                new FuncCondition<MyContext>(
                    "Can not choose A",
                    context => context.GetState(MyWorldState.HasA) == 0
                )
            );
            actionB.SetOperator(new MyOperator());
