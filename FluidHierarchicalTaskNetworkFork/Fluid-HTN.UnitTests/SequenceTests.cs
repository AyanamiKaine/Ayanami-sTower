...
namespace Fluid_HTN.UnitTests
{
    ...
    public class SequenceTests
    {
        ...
        public void DecomposeNestedSubtasks_ExpectedBehavior()
        {
            ...
            task3.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task1" }.AddCondition(
                    new FuncCondition<MyContext>("Done == true", context => context.Done == true)
                )
            );
        ...
        public void DecomposeWithSubtasksOneFail_ExpectedBehavior()
        {
            ...
            task.AddSubtask(new PrimitiveTask() { Name = "Sub-task1" });
            task.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task2" }.AddCondition(
                    new FuncCondition<MyContext>("Done == true", context => context.Done == true)
                )
            );
            var status = task.Decompose(ctx, 0, out var plan);
        ...
        public void DecomposeFailureReturnToPreviousWorldState_ExpectedBehavior()
        {
            ...
            var task = new Sequence() { Name = "Test" };
            task.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task1" }.AddEffect(
                    new ActionEffect<MyContext>(
                        "TestEffect",
                        EffectType.Permanent,
                        (context, type) =>
                            context.SetState(MyWorldState.HasA, false, EffectType.PlanOnly)
                    )
                )
            );
        ...
        public void DecomposeNestedCompoundSubtaskLoseToMTR2_ExpectedBehavior()
        {
            ...
            task2.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task3" }.AddCondition(
                    new FuncCondition<MyContext>("Done == true", context => context.Done == true)
                )
            );
            task2.AddSubtask(task3);
        ...
        public void DecomposeNestedCompoundSubtaskLoseToMTRReturnToPreviousWorldState_ExpectedBehavior()
        {
            ...
            task3.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task3" }.AddEffect(
                    new ActionEffect<MyContext>(
                        "TestEffect",
                        EffectType.Permanent,
                        (context, type) =>
            ...
            task2.AddSubtask(task3);
            task2.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task4" }.AddEffect(
                    new ActionEffect<MyContext>(
                        "TestEffect",
                        EffectType.Permanent,
                        (context, type) =>
                            context.SetState(MyWorldState.HasB, false, EffectType.PlanOnly)
                    )
                )
            );
            ...
            task.AddSubtask(task2);
            task.AddSubtask(
                new PrimitiveTask() { Name = "Sub-task5" }.AddEffect(
                    new ActionEffect<MyContext>(
                        "TestEffect",
                        EffectType.Permanent,
                        (context, type) =>
                            context.SetState(MyWorldState.HasC, false, EffectType.PlanOnly)
                    )
                )
            );
