...
export interface StoredDoc { id: number; text: string; embedding: ArrayBuffer; summary?: string | null; summary_embedding?: ArrayBuffer | null; tags?: string[] | null; created_at: number; updated_at: number; }

export function insertDoc(text: string, embedding: Float32Array, id?: number, summary?: string, summaryEmbedding?: Float32Array | null, tags?: string[] | null): number {
  const db = getDB();
  ...
  const stmt = db.prepare('INSERT INTO docs(id,text,embedding,summary,summary_embedding,tags,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)');
  const newId = id ?? ((db.prepare('SELECT COALESCE(MAX(id),0)+1 AS nid FROM docs').get() as any).nid);
  stmt.run(newId, text, buf, summary ?? null, sumBuf, tagsJson, ts, ts);
  return newId;
}

export function updateDoc(id: number, text: string, embedding: Float32Array) {
  const db = getDB();
  const ts = Math.floor(Date.now() / 1000);
  const buf = Buffer.from(new Uint8Array(embedding.buffer.slice(0)));
  const res = db.prepare('UPDATE docs SET text=?, embedding=?, updated_at=? WHERE id=?').run(text, buf, ts, id);
...
export function getDoc(id: number): StoredDoc | null {
  const db = getDB();
  const row = db.prepare('SELECT id, text, embedding, summary, summary_embedding, tags, created_at, updated_at FROM docs WHERE id=?').get(id) as any;
  if (!row) return null;
  if (row.tags) {
...
export interface ListedDoc { id:number; text:string; summary?:string|null; tags?:string[]|null; created_at:number; updated_at:number; }
...
export function listDocs(opts: { limit?: number; offset?: number; order?: 'recent'|'id' } = {}): ListedDoc[] {
  ...
  const rows = db.prepare(`SELECT id, text, summary, tags, created_at, updated_at FROM docs ORDER BY ${order} LIMIT ? OFFSET ?`).all(limit, offset) as any[];
  rows.forEach(r => { if (r.tags) { try { r.tags = JSON.parse(r.tags); } catch { r.tags = null; } } });
  return rows as ListedDoc[];
}

export interface Retrieved { id: number; text: string; summary?: string | null; tags?: string[] | null; distance: number; created_at: number; updated_at: number; }
...
export function search(embedding: Float32Array, topK: number, useRecency: boolean, halfLife: number, alpha: number, opts?: { useSummary?: boolean; fallbackFull?: boolean }): Retrieved[] {
  ...
  if (!useSummary) {
    const stmt = db.prepare('SELECT id, text, summary, tags, vec_distance_l2(embedding, ?) AS distance, created_at, updated_at FROM docs ORDER BY distance ASC LIMIT ?');
    const rows = stmt.all(buffer, limit) as any[];
    rows.forEach(r => { if (r.tags) { try { r.tags = JSON.parse(r.tags); } catch { r.tags = null; } } });
    if (!useRecency) return rows.map(r => ({ id: r.id, text: r.text, summary: r.summary, tags: r.tags, distance: r.distance, created_at: r.created_at, updated_at: r.updated_at }));
    const now = Math.floor(Date.now() / 1000);
    const rescored = rows.map(r => {
      ...
      return { adjusted, r };
    }).sort((a,b) => a.adjusted - b.adjusted).slice(0, topK).map(x => ({ id: x.r.id, text: x.r.text, summary: x.r.summary, tags: x.r.tags, distance: x.r.distance, created_at: x.r.created_at, updated_at: x.r.updated_at }));
    return rescored;
  }
  // Summary search path
  const rows = db.prepare('SELECT id, text, summary, tags, summary_embedding, embedding, created_at, updated_at FROM docs WHERE summary_embedding IS NOT NULL').all() as any[];
  ...
  if (!useRecency) {
    return distances.slice(0, topK).map(x => ({ id: x.r.id, text: x.r.text, summary: x.r.summary, tags: x.r.tags, distance: x.d, created_at: x.r.created_at, updated_at: x.r.updated_at }));
  }
  const now = Math.floor(Date.now()/1000);
  ...
  }).sort((a,b)=> a.adjusted - b.adjusted).slice(0, topK).map(x => ({ id: x.r.id, text: x.r.text, summary: x.r.summary, tags: x.r.tags, distance: x.original, created_at: x.r.created_at, updated_at: x.r.updated_at }));
  return rescored;
}
