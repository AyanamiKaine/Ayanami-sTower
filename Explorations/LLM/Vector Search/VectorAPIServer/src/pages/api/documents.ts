...
export const POST: APIRoute = async ({ request }) => {
  ...
  const embedding = await embedText(body.text, { isQuery: false });
  ...
  if (body.auto_summarize) {
    try { 
      ...
      summaryEmbedding = await embedText(summary, { isQuery: false });
    } catch {}
  }
  if (body.auto_tag) {
  ...
  try {
    ...
    if (body.resummarize) {
      try {
        ...
        try { summaryEmbedding = await embedText(summary, { isQuery: false }); } catch {}
